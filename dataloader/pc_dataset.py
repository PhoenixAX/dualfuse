...
class SemanticKITTI(data.Dataset):
    def __init__(self, config, data_path, imageset='train', num_vote=1):
        ...
        for i_folder in split:
            self.im_idx += absoluteFilePaths('/'.join([data_path, str(i_folder).zfill(2), 'sem_velodyne']), num_vote)   ##sem_
    ...
    def __getitem__(self, index):
        raw_data = np.fromfile(self.im_idx[index], dtype=np.float32).reshape((-1, 5))##4
        origin_len = len(raw_data)
        points = raw_data[:, :3]
        sem=raw_data[:, 4:5]/255##
        ...
        if self.imageset == 'test':
            ...
            annotated_data = np.fromfile(self.im_idx[index].replace('sem_velodyne', 'labels')[:-3] + 'label',##sem_
        ...
        image_file = self.im_idx[index].replace('sem_velodyne', 'image_2').replace('.bin', '.png')##sem_
        image = Image.open(image_file)
        proj_matrix = self.proj_matrix[int(self.im_idx[index][-26:-24])]##[-22:-20])]
        ...
        ##
        data_dict['sem_project']=sem#raw_data[:, 4:5]
