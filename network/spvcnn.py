...
from network.vfe import  SphereFormer


class point_encoder(nn.Module):
    def __init__(self, in_channels, out_channels, scale,layer_id):
    ...
    def forward(self, features, data_dict):
        ...
        ###
        #output=self.att(output, data_dict['ref_xyz'],data_dict['batch_idx'])
        #output =self.reshape_(output)
        ###
...
class SPVBlock(nn.Module):
    def __init__(self, in_channels, out_channels, indice_key, scale, last_scale, spatial_shape):
        ...
        self.p_enc = point_encoder(in_channels, out_channels, scale,self.layer_id )
    ...
    def forward(self, data_dict):
        ...
        #v_fea_inv=self.reshape_1(v_fea_inv)
        #v_fea_inv=self.att(v_fea_inv, data_dict['ref_xyz'],data_dict['batch_idx'])
        #v_fea_inv=self.reshape_2(v_fea_inv)
        ...
        return  p_fea[coors_inv]##self.reshape_2(self.att(self.reshape_1(p_fea[coors_inv]), data_dict['ref_xyz'],data_dict['batch_idx']))#
...
class get_model(LightningBaseModel):
    def __init__(self, config):
        ...
        self.classifier = nn.Sequential(
            nn.Linear(self.hiden_size * self.num_scales//2, 128),
            nn.ReLU(True),
        ...
        #SF
        #'''
        dpr = [x.item() for x in torch.linspace(0, 0.3, 7)]
        voxel_size=[0.05, 0.05, 0.05]
        patch_size =np.array([voxel_size[i] * 1 for i in range(3)]).astype(np.float32)
        window_size = patch_size *6
        window_size_sphere=np.array([2, 2, 80])
        sf_c=32
        self.reshape_1 = nn.Sequential(
            nn.Linear(self.hiden_size*4 , sf_c),
            nn.ReLU(True),
            #nn.Linear(self.hiden_size//2, self.hiden_size//2)
            )
        self.att=SphereFormer(
               sf_c, 
                (sf_c)//16, 
                ...
                indice_key='sphereformer{}'.format(0),
                rel_query=True, 
                rel_key=True, 
                rel_value=True, 
                drop_path=dpr[0],
        ...
        self.reshape_2 = nn.Sequential(
            nn.Linear(sf_c , self.hiden_size*2),
            nn.ReLU(True),
    ...
    def forward(self, data_dict):
        ...
        output=self.reshape_2(self.att(self.reshape_1(output),data_dict['ref_xyz'],data_dict['batch_idx']))
